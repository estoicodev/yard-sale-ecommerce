...
function MyAccount() {
  const { userAccount } = useOnlineStore();
  ...
  return (
    <section className="w-full max-w-sm h-full mx-auto flex flex-col py-10 relative login">
      ...
      <form action="" className="w-full flex flex-col login__form">
        ...
        <p className="h-12 sm:h-14 mb-5 text-[#c7c7c7] text-base font-normal value">{userAccount.name}</p>
        <label htmlFor="email" className="mb-4 text-base font-bold login__label">Email address</label>
        <p className="h-12 sm:h-14 mb-5 text-[#c7c7c7] text-base font-normal value">{userAccount.email}</p>
